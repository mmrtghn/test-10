...
const upload = multer({ storage: multer.memoryStorage(), limits: { fileSize: MAX_UPLOAD_BYTES, files: MAX_UPLOAD_FILES, fields: 4, fieldSize: 32_000 } });
app.post("/api/application", (req, res, next) => upload.any()(req, res, (error) => { if (error instanceof multer.MulterError) { if (error.code === "LIMIT_FILE_SIZE") return sendError(res, 413, "One of the uploaded files is too large.", "UPLOAD_FILE_TOO_LARGE"); if (error.code === "LIMIT_FILE_COUNT") return sendError(res, 400, "Too many files were uploaded.", "UPLOAD_COUNT_EXCEEDED"); return sendError(res, 400, "The uploaded files could not be accepted.", "UPLOAD_INVALID"); } if (error) return next(error); next(); }), async (req, res, next) => {
  try {
    const configuration = await readConfiguration();
    ...
    if (!uploads.valid) return sendError(res, uploads.status || 400, uploads.message, uploads.code);
  ...
  } catch (error) { console.error("Application delivery failed:", safeError(error)); sendError(res, 503, "The application could not be delivered right now. Please try again later.", "DELIVERY_FAILED"); }
});
...
function validateUploads(files, community) {
  const expected = new Set(community.animals.map((a) => `animal_${a.id}`));
  const allowed = new Set(["experience", ...expected]);
  const seen = new Set();
  const output = [];
  for (const file of files) {
    if (!allowed.has(file.fieldname) || seen.has(file.fieldname)) return uploadError("The uploaded file list is not valid.", "UPLOAD_INVALID");
    ...
    const accepted = String(settings.accept).split(",").map((x) => x.trim().toLowerCase());
    const mime = detectMime(file.buffer);
    const expectedMime = { ".pdf": "application/pdf", ".png": "image/png", ".jpg": "image/jpeg", ".jpeg": "image/jpeg", ".webp": "image/webp" }[ext];
    if (!name || name.length > 120 || file.size < 1 || file.size > Math.min(settings.maxBytes, MAX_UPLOAD_BYTES) || !accepted.includes(ext) || mime !== expectedMime || (file.mimetype && file.mimetype !== mime && !(mime === "image/jpeg" && file.mimetype === "image/jpg"))) return uploadError("One of the uploaded files is invalid.", "UPLOAD_INVALID");
    output.push({ field: file.fieldname, name, size: file.size, mime, buffer: file.buffer });
  }
  if (!seen.has("experience") || [...expected].some((x) => !seen.has(x))) return uploadError("Attach all required files before sending.", "UPLOAD_REQUIRED");
  if (output.reduce((sum, f) => sum + f.size, 0) > MAX_TOTAL_UPLOAD_BYTES) return uploadError("The combined upload size is too large.", "UPLOAD_TOTAL_TOO_LARGE", 413);
  return { valid: true, files: output };
}
function uploadError(message, code, status = 400) { return { valid: false, message, code, status }; }
...
function sendError(res, status, message, code = "") { return res.status(status).json({ message, ...(code ? { code } : {}) }); }
