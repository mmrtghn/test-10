...
export function migrateConfiguration(input, { onWarning } = {}) {
  if (input?.schemaVersion === CONFIG_SCHEMA_VERSION) return structuredClone(input);
  const legacyCommunities = getLegacyCommunities(input);
  ...
  for (const [id, legacy] of Object.entries(legacyCommunities)) {
    ...
    const template = pickTemplate(legacy, `communities.${id}`, onWarning);
    ...
    for (const key of ["math", "date"]) {
      if (Object.prototype.hasOwnProperty.call(legacy.steps || {}, key)) {
        onWarning?.({ path: `communities.${id}.steps.${key}`, code: "migration.dropped-field", message: `Retired field ${key} was removed during migration.` });
      }
    }
    for (const key of ["mathIntro", "mathHint", "dateIntro", "dateHint"]) {
      if (Object.prototype.hasOwnProperty.call(legacy.captcha || {}, key)) {
        onWarning?.({ path: `communities.${id}.captcha.${key}`, code: "migration.dropped-field", message: `Retired field ${key} was removed during migration.` });
      }
    }
  }
...
export function getLegacyCommunities(input) {
  return isObject(input?.communities) ? input.communities : isObject(input) ? input : null;
}

export function extractLegacyWebhook(input) {
  const webhooks = new Set();
  for (const community of Object.values(getLegacyCommunities(input) || {})) {
    if (isObject(community) && text(community.webhook)) webhooks.add(community.webhook.trim());
  }
  if (webhooks.size > 1) throw new Error("Legacy communities contain different webhooks; migrate manually.");
  return [...webhooks][0] || "";
}
...
function normalizeLegacyUploads(value, prefix, onWarning) {
  const defaults = createDefaultTemplate().allowedUploads;
  const uploads = {};
  for (const type of ["application", "animal"]) {
    const setting = isObject(value?.[type]) ? value[type] : {};
    const path = `${prefix}.${type}`;
    const extensions = String(setting.accept || "")
      .split(",")
      .map((extension) => extension.trim().toLowerCase())
      .filter((extension, index, values) => extension && values.indexOf(extension) === index && UPLOAD_EXTENSIONS[type].has(extension));
    const accept = extensions.length ? extensions.join(",") : defaults[type].accept;
    if (accept !== setting.accept) {
      onWarning?.({ path: `${path}.accept`, code: "migration.upload-accept", message: "Legacy upload extensions were normalized to supported values." });
    }
    const numeric = typeof setting.maxBytes === "string" && setting.maxBytes.trim() !== ""
      ? Number(setting.maxBytes)
      : setting.maxBytes;
    const normalized = Number.isFinite(numeric) ? Math.trunc(numeric) : NaN;
    const maxBytes = Number.isSafeInteger(normalized) && normalized > 0
      ? Math.min(normalized, MAX_UPLOAD_BYTES)
      : MAX_UPLOAD_BYTES;
    if (maxBytes !== setting.maxBytes) {
      onWarning?.({ path: `${path}.maxBytes`, code: "migration.upload-size", message: `Legacy upload size was normalized to ${maxBytes} bytes.` });
    }
    uploads[type] = {
      accept,
      label: text(setting.label) ? setting.label : defaults[type].label,
      maxBytes
    };
    if (!text(setting.label)) {
      onWarning?.({ path: `${path}.label`, code: "migration.upload-label", message: "Legacy upload label was replaced with the default." });
    }
  }
  return uploads;
}
